...
client.on("voiceStateUpdate", async (oldVoice, newVoice) => {
  ...
  if (oldVoice.channel && !oldVoice.channel.members.size && oldVoice.channel.parent.id == config.gamecategoryid && oldVoice.channelID !== config.newgamevcid) oldVoice.channel.delete();
})
